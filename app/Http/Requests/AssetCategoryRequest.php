...
class AssetCategoryRequest extends FormRequest
{
    ...
    {
        return [
            'category_name' => 'required|unique:asset_categories,category_name|string|max:100',
    ...
    protected function failedValidation(Validator $validator)
    {
        if ($this->isMethod('post')) {
            ...
            $errorMessage = 'Sorry, AssetCategory creation failed';
        } elseif ($this->isMethod('put')) {
            // For update failures
            $errorMessage = 'Sorry, AssetCategory update failed';
        } else {
